...
void Defender::print_with_details(int index) {
...
void Defender::calculate_score(Match* match) {
    int raw_score = DEF_INITIAL_PTS;
    ...
    if (current_stats->did_cleansheet()) raw_score += DEF_CLEANSHEET_PTS;
    raw_score += current_stats->get_goals() * DEF_SCORE_PTS;
    raw_score += current_stats->get_assists() * DEF_ASSIST_PTS;
